...
EXPECTED_CURRENT_OWNER_SENTENCE = "Phase67 is `ACTIVE`, N67=16; Slices01–06 deliver CI/Arrow readiness, canonical result descriptions, checked scalar/Text batches, and exact finite Decimal128/256 results over the approved shared precision-65 parameter domain."
...
def test_phase65_slice3_current_binding_and_whole_plan_boundary() -> None:
    status = " ".join(_read(STATUS).split())
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = "spec/phase65-slice3-named-producer-graph-repeated-imported-uses-scope-local-symbols-v1.md"
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
        "13 definitions and 24 uses",
...
def test_phase65_slice5_current_matching_and_obligation_boundary() -> None:
    ...
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = (
        "spec/phase65-slice5-seven-join-kinds-match-scopes-obligation-retention-v1.md"
    )
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
...
def test_phase65_slice6_current_aggregate_boundary() -> None:
    status = " ".join(_read(STATUS).split())
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = "spec/phase65-slice6-grouped-global-satisfying-block-boundaries-v1.md"
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
        "grouping determinants",
        "canonical exports",
        "one-row-on-empty-input",
        "exact-ref inspection",
    ):
        assert phrase in status and phrase in roadmap
    assert "That publication left Slice14 next; N16 stayed unchanged." in status


def test_phase65_slice7_current_window_and_qualify_boundary() -> None:
    status = " ".join(_read(STATUS).split())
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = "spec/phase65-slice7-windows-named-window-qualify-staging-v1.md"
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
        "selected/hidden window computations",
        "named-window provenance",
        "SQL TRUE-only membership",
        "NEW construction-time hidden binding preparation",
        "immediate active exports",
        "origins/demands",
    ):
        assert phrase in status and phrase in roadmap
    assert "That publication left Slice14 next; N16 stayed unchanged." in status
...
def test_phase65_slice8_current_result_boundary() -> None:
    ...
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = (
        "spec/phase65-slice8-distinct-scoped-order-static-limit-result-boundaries-v1.md"
    )
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
        "visible-row DISTINCT",
        "pending hidden STRICT-FD realization",
        "NEW construction-time ORDER preparation",
        "RIGHT_LIMIT proof transport",
...
def test_phase65_slice10_current_fixed_literal_boundary() -> None:
    status = " ".join(_read(STATUS).split())
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = "spec/phase65-slice10-typed-fixed-literal-envelope-bind-use-layout-v1.md"
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
        "complete contextual literal dispositions",
        "typed fixed-value envelope",
        "actual slot-backed expression uses",
        "exact numeric tags and signed zero",
        "Independent fixed-envelope verification",
...
def test_phase65_slice11_current_requirement_report_boundary() -> None:
    status = " ".join(_read(STATUS).split())
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = "spec/phase65-slice11-complete-demand-obligation-report-v1.md"
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
        "complete demand/obligation report",
        "Original demand occurrences",
        "captured literal policy and envelope",
        "Immutable exact-ref queries",
        "PROVED and LEGAL_UNPROVED",
        "pending hidden ORDER realization",
        "Target posture remains NOT_ASSESSED",
...
def test_phase65_slice13_current_target_assessment_boundary() -> None:
    status = " ".join(_read(STATUS).split())
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = "spec/phase65-slice13-explicit-target-profile-requirement-assessment-v1.md"
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
        "Canonical compiler subpropositions, raw profile/provider lookup and declared applicability remain separate",
        "original enforcement/proof/risk and hidden ORDER records remain pending",
        "Two exact private consumers are registered in existing privacy guards",
    ):
        assert phrase in status and phrase in roadmap
    assert "That publication left Slice14 next; N16 stayed unchanged." in status


def test_phase65_slice14_current_portable_boundary() -> None:
    status = " ".join(_read(STATUS).split())
    roadmap = " ".join(_section(_read(ROADMAP), "Phase 65 route").split())
    target = "spec/phase65-slice14-portable-boundary-minimal-process-integration-v1.md"
    assert f"]({target})" in status and f"]({target})" in roadmap
    assert (STATUS.parent / target).is_file()
    for phrase in (
        "original-object correspondence and independent pure decoding",
        "Pure consistency does not authenticate runtime roots or certify database support or execution",
...
def test_phase66_slice7_join_publication_preserves_later_boundaries() -> None:
    target = (
        "spec/phase66-slice7-value-bridge-join-exists-outer-nulling"
        "-terminal-output-emission-v1.md"
    )
    for path in (STATUS, ROADMAP):
        document = " ".join(_read(path).split())
        assert f"]({target})" in document
        assert "Phase66 Slice7 is " in document
        assert "only upon successful natural exact-head CI" in document
        assert "strict v2 aggregate" in document
        assert "W–V25 cases/69 public documents" in document
        assert (
            "postgres49 VERIFIED,3 INPUT_REJECTED,17 BLOCKED" in document
            and "mysql48 VERIFIED,3 INPUT_REJECTED,18 BLOCKED" in document
        )
        # The later owners of the two amended ledgers stay visible here.
        assert (
        ...
        assert "Do not start Slice8 automatically" in document
        assert "N66 remains16" in document
    assert (STATUS.parent / target).is_file()
...
def test_phase66_slice12_is_recorded_with_its_unchanged_denominator() -> None:
    target = (
        ...
        "sql-range-queries-v1.md"
    )
    for path in (STATUS, ROADMAP):
        document = " ".join(_read(path).split())
        assert f"]({target})" in document
        assert (
            "Phase66 Slice12 is `COMPLETED / PUBLISHED` only upon successful"
            " natural exact-head CI" in document
        )
        assert (
            "one private immutable inspection view binds the exact verified"
            " runtime artifact" in document
        )
        assert (
            "refuses malformed, stale, foreign and decoded-document roots" in document
        )
        assert (
            "independently re-enumerated from the retained plan demands and from"
            " the actual generated SQL structures" in document
        )
        assert "tokens before overlays" in document
        assert "include `project_sql_emission_inspection`" in document
        assert (
            "The denominator is unchanged: 60 cases/181 public documents per"
            " target: postgres150 VERIFIED,3 INPUT_REJECTED,28 BLOCKED; mysql147"
            " VERIFIED,3 INPUT_REJECTED,31 BLOCKED." in document
        )
        assert (
            "Phase66 remains `ACTIVE`; Slice13 is `NEXT / NOT IMPLEMENTED`;"
            " Slices14–16 remain `NOT IMPLEMENTED`." in document
        )
        assert "Do not start Slice13 automatically" in document
        assert "Slice12 completion is not Phase66 completion" in document
    assert (STATUS.parent / target).is_file()
    route = (
        STATUS.parent
        / "spec"
    ...
    document = " ".join(route.read_text(encoding="utf-8").split())
    assert "## R23 Slice12 delivery" in document
    assert "project_sql_emission_inspection" in document
...
def test_phase66_slice7_amendments_are_recorded_in_the_route_lock() -> None:
    ...
    document = " ".join(route.read_text(encoding="utf-8").split())
    assert "## R11/C09 scheduling amendment authorized for Slice7" in document
    assert (
...
def test_g8_is_unnumbered_and_does_not_complete_the_slice16_audit() -> None:
    target = "spec/phase66-pre-slice16-g8-window-qualify-native-membership-evidence-closure-v1.md"
    for path in (STATUS, ROADMAP):
        document = " ".join(_read(path).split())
        assert f"]({target})" in document
        for phrase in (
...
def test_phase67_slice06_keeps_the_next_temporal_slice_unstarted() -> None:
    for path in (STATUS, ROADMAP):
        document = _read(path)
        ...
        assert (
            "Slice07 `NEXT / NOT STARTED`" in current
            and "Slices08–16 `NOT STARTED`" in current
        )
        assert "phases/phase-67/slice-06.md" in current
