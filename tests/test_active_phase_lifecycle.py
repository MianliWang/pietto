...
EXPECTED_STATUS = (
    ...
    (
        "Next",
        "`PHASE59_SLICE4_REQUIREMENT_SELECTOR_ATTRIBUTION_END_TO_END`",
    ),
)
EXPECTED_PHASE58_STATE = "All 17 slices are completed. Phase 58 is complete."
EXPECTED_PHASE59_STATE = (
    "Phase 59 is active, Slices 1–2 are completed, Slice 3 is current, and "
    "Slice 4 is next / unstarted. The published route has exactly 12 slices."
)
...
def test_active_status_table_and_authority_prose_are_exact() -> None:
    ...
    assert "Slice 3 is the current canonical package-graph construction owner" in (
        normalized
    )
    assert "Live Git and natural exact-head CI own Phase 59 Slice 3 completion" in (
        normalized
    )
