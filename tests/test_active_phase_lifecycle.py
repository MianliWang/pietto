...
EXPECTED_CURRENT_OWNER_SENTENCE = "Phase67 is `ACTIVE`, N67=16; Slices01–05 deliver CI/Arrow readiness, canonical result descriptions, checked Int/Bool/finite Float batches, and exact Unicode Text with explicit string/large_string representation."
...
def test_phase67_slice05_keeps_the_next_decimal_slice_unstarted() -> None:
    for path in (STATUS, ROADMAP):
        document = _read(path)
        ...
        assert (
            "Slice06 `NEXT / NOT STARTED`" in current
            and "Slices07–16 `NOT STARTED`" in current
        )
        assert "phases/phase-67/slice-05.md" in current
