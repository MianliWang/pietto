...
def test_ranking_placements_outside_direct_select_fail_closed(
    ...
    assert semantic_source.count("analyze_ranking_window_expression(") == 1
...
def test_peer_and_project_facts_are_transient_not_model_state(
    ...
    if case == 0:
        assert "analyze_ranking_window_expression(" in semantic_source
        assert "ranking_window_facts:" not in semantic_source
    else:
        assert "build_ranking_window_result_project_fact(" in project_source
...
@pytest.mark.parametrize("name", ("percent_rank", "cume_dist", "ntile", "lag", "lead"))
...
def test_test_inventory_focused_selector_dirty_overlay_and_formatter_are_exact() -> (
    ...
    assert (len(FOCUSED_OPERANDS), len(focused_payload)) == (113, 12876)
    assert hashlib.sha256(focused_payload).hexdigest() == FOCUSED_SHA256
    assert len({item.split("::", 1)[0] for item in FOCUSED_OPERANDS}) == 66
    assert sum("::" not in item for item in FOCUSED_OPERANDS) == 7
    ...
    assert len(FORMATTER_PATHS) == len(set(FORMATTER_PATHS)) == 60
    assert len(formatter_payload) == 2990
