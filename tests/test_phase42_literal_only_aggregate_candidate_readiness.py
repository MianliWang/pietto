...
def test_phase42_slice6_plan_alignment_and_guardrails_are_documented() -> None:
    ...
    for required in (
        ...
        "Aggregate typeclass, literal-only aggregate behavior, grouped let keys, grouped let ordering, raw `satisfying` let-name behavior, and `limit let_name` behavior unfreeze only when a later implementation slice is explicitly approved",
    ):
