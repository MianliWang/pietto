...
PHASE54_WORKFLOW_EFFICIENCY_SUBJECT = (
    "Add Pietto lean end-to-end workflow infrastructure"
)
...
def _phase54_gate2_paths(name: str) -> set[str]:
    path = REPO_ROOT / "tests/_active_gate2_manifest_data.py"
    tree = ast.parse(path.read_text(encoding="utf-8"))
    assignments = {
...
def _is_clean_projection() -> bool:
    ...
    if parents == (PHASE54_SLICE6_HEAD,) and _is_phase54_subject(
        ...
        if pull_request_identity is not None:
            ...
            assert candidate_sha == head
            assert pull_request_refs == (
                "main",
                PHASE54_WORKFLOW_EFFICIENCY_BRANCH,
            )
            return True
        if os.environ.get("GITHUB_EVENT_NAME") == "push":
            assert shallow == "true"
        ...
        assert shallow == "false"
        branch = _git_output(["branch", "--show-current"])
        if branch == PHASE54_WORKFLOW_EFFICIENCY_BRANCH:
    ...
    if pull_request_identity is not None:
        ...
        if base_sha == PHASE54_SLICE5_HEAD or candidate_ref == PHASE54_SLICE6_BRANCH:
            ...
            return True
        if (
            base_sha == PHASE54_SLICE6_HEAD
            or candidate_ref == PHASE54_WORKFLOW_EFFICIENCY_BRANCH
        ):
            assert base_sha == PHASE54_SLICE6_HEAD
            assert base_ref == "main"
            assert candidate_ref == PHASE54_WORKFLOW_EFFICIENCY_BRANCH
            assert head != candidate_sha
