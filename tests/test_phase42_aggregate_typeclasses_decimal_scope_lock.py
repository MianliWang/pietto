...
def test_current_maps_and_findings_are_documented() -> None:
    ...
    for required in (
        ...
        "`Decimal * Decimal`, mixed `Decimal`/`Int`, mixed `Decimal`/`Float`",
...
def test_future_sequence_forbidden_surfaces_and_stop_conditions_are_locked() -> None:
    ...
    for required in (
        ...
        "| 4 | Decimal Precision Fusion Candidate |",
        "| 5 | Literal-only Aggregate Argument Candidate |",
...
def test_deferred_register_classifies_phase42_related_future_work_only() -> None:
    ...
    for required in (
        ...
        "It implements no aggregate behavior",
        "Phase 42 Slice 1 scope-lock records the current numeric expression typing map",
        "It implements no Decimal arithmetic behavior",
        "Phase 42 Slice 1 classifies aggregate typeclass, Decimal arithmetic, literal-only aggregate, and precision-fusion prerequisites",
