...
def test_no_group_count_distinct_lowers_to_one_arg_aggregate_ir(
    relation_kind: str,
) -> None:
    script = _parse(
    ...
    ] == [("unique_customers", "Int", NullabilityIR.NON_NULL)]


def test_no_group_qualified_count_distinct_lowers_to_qualified_field_ref() -> None:
    script = _parse(
...
def test_grouped_count_distinct_lowers_with_group_keys_preserved() -> None:
    script = _parse(
    ...
    _assert_aggregate(
        projections["unique_customers"].expression,
        "count_distinct",
        "Int",
        NullabilityIR.NON_NULL,
        ("customer_id",),
...
def test_grouped_qualified_count_distinct_lowers_with_group_keys_preserved() -> None:
    script = _parse(
        SOURCE_PREFIX + "table unique_customers_by_status:\n"
        "    from orders\n"
        "    group by:\n"
...
def test_direct_lower_expr_for_valid_count_distinct_uses_aggregate_call_ir() -> None:
    script = _parse(
        SOURCE_PREFIX + "table unique_order_values:\n"
        "    from orders\n"
        "    select:\n"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "    from orders\n"
            "    select:\n"
            "        unique_values = count_distinct(raw)\n",
            "PIE-S2314",
        ),
        (
            SOURCE_PREFIX + "table unique_order_values:\n"
            "    from orders\n"
            "    select:\n"
            "        unique_values = count_distinct(payload)\n",
            "PIE-S2314",
        ),
        (
            SOURCE_PREFIX + "table unique_order_values:\n"
            "    from orders\n"
            "    select:\n"
            "        unique_values = count_distinct(anything)\n",
            "PIE-S2314",
        ),
        (
            SOURCE_PREFIX + "table unique_order_values:\n"
...
def test_invalid_count_distinct_shapes_do_not_emit_aggregate_call_ir(
    source: str,
    expected_code: str,
) -> None:
...
def test_existing_aggregate_ir_behavior_remains_unchanged() -> None:
    script = _parse(
