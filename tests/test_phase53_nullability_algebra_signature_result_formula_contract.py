...
def test_concrete_semantic_project_and_aggregate_nullability_authority_is_locked() -> (
    ...
    expected = {
        ...
        "src/pietto/_project/model.py": "9bf037d5b529a7d1a592271b8a48e3401a403c8cc3b285411977035f28c93b9c",
...
def test_test_inventory_focused_selector_and_dirty_overlay_are_exact() -> None:
    repository_paths = _all_repository_paths()
    assert len(repository_paths) == 909
    assert sum(path.endswith(".py") for path in repository_paths) == 559
    assert sum(path.endswith(".md") for path in repository_paths) == 254
    test_paths = tuple(sorted((REPO_ROOT / "tests").glob("test_*.py")))
    assert len(test_paths) == 457
    functions = tuple(
        node.name
        for path in test_paths
