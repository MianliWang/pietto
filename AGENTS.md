...
1 and 2 are complete. Slices 3 through 6 are planned only and require
...
and scope-aware documentation. It does not implement `LIMIT` or `ORDER BY`
and changes no production behavior, grammar, generated ANTLR, AST, semantic
analysis, IR, SQL backend, CLI, JSON schema, or public Python API.
...
`docs/plan/phase-12-sql-feature-expansion-i.md`. Slices 1 and 2 are complete.
Slices 3 through 6 are planned only and are not authorized by their inclusion
...
Phase 12 Slices 1 and 2 are complete as planning, baseline audit, and language
contract only. Slices 3 through 6 remain planned and unimplemented.
