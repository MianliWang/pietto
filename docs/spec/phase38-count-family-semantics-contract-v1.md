...
| `count_if(predicate)` | No current aggregate or builtin function surface; future candidate only. | `docs/plan/phase-38-aggregate-semantics-and-type-capability-consolidation.md`, `src/pietto/semantic/aggregates.py`, `src/pietto/semantic/catalog.py` |
...
| `count_distinct(field)` | Accepted for current direct-field subset `Bool`, `Int`, `Float`, `Decimal`, `Text`, `Date`, `Timestamp`, and `UUID`; result is `Int not null`. | `docs/plan/phase-38-aggregate-semantics-and-type-capability-consolidation.md`, `src/pietto/semantic/aggregates.py`, `tests/test_phase31_aggregate_result_matrix_hardening.py` |
