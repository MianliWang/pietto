...
`mysql` remains disabled until Phase 10 Slice 8. `emit_mysql_sql`,
`mysql.table`, and `--dialect mysql` are not implemented by this design.
...
- `emit_mysql_sql` and `mysql.table` are absent until their own slices;
