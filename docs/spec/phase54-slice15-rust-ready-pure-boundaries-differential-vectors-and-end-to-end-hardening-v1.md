...
| `source_shape_resolution`, `relation_resolution` | the owner is a declaration of this module of the kind the record admits, a shaped source or a table or query; owners ascend, so one declaration resolves once in source order; the resolved target is one entry of this module's origin catalog, by target module path and declared name, of the kind the reference admits — a shape for a source shape, a source or table or query for a relation — and a relation resolution matches its local name there too | `ProjectModuleSourceShapeReference`, `ProjectModuleRelationReference` |
...
| `type_resolution_alias` | every alias identity is a type alias in the type namespace, no identity repeats in one chain, and an alias in this module is one type declaration that module publishes | `ProjectResolvedModuleTypeReference` |
