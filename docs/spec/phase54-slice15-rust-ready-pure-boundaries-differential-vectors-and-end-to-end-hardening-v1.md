...
| ledger excludes | one record against a collection the enclosing scope already published, refusing the tuples it contains | a local facade entry is the module's only candidate, so no import request binds that same name, namespace, and kind |
