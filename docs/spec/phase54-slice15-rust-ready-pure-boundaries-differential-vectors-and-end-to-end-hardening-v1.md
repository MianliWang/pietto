...
| `module` | every selected module path appears once, every module identity path in the document is a normalized project-relative `.pietto` path, and every graph import evidence names one of the module's own import requests | `ProjectLogicalModule`, `ProjectModuleIdentity` |
...
| `row_lineage` | the owner is a relation declaration of this module; the status and reason are one of the pairs a state builder can publish; a non-concrete lineage carries no field; and owners ascend, so one relation publishes one lineage in source order | `ProjectModuleRelationLineage`, `ProjectModuleAttributionFactSet` |
...
| `semantic_facts` | the state is the state its own declaration retained, because both project one Slice 12 fact, unless that declaration retained none at all; a state pair no source builder reaches names a table or query owner; the owner is a relation declaration of this module, a loader-blocked module publishes no fact set at all, and a fact set carrying any derived product names a table or query, since a source keeps its base row state and publishes none; the status and reason are one of the pairs a state builder can publish; owners ascend, so one relation publishes one fact set in source order, every window output names a select of the same fact set, and one ambiguous clause dependency blocks the relation for conflicting facts and blocks every window output | `ProjectModuleSemanticFactEnvironment`, `ProjectModuleRelationSemanticFacts` |
