...
| deferred ledger match | one record against a collection the enclosing scope publishes after it, settled when that scope closes | a local facade entry names a declaration of its own module, which the module publishes in a later section |
...
A type resolution's canonical target is not matched against the origin catalog,
and a fixture refutes the rule directly. A module that imports an alias whose
