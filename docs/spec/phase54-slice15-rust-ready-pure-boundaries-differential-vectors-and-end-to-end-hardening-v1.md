...
| `semantic_facts` | owners ascend, so one relation publishes one fact set in source order, every window output names a select of the same fact set, and one ambiguous clause dependency blocks the relation for conflicting facts and blocks every window output | `ProjectModuleSemanticFactEnvironment`, `ProjectModuleRelationSemanticFacts` |
