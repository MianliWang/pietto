...
| `row_lineage` | the status and reason are one of the pairs a state builder can publish; a non-concrete lineage carries no field; and owners ascend, so one relation publishes one lineage in source order | `ProjectModuleRelationLineage`, `ProjectModuleAttributionFactSet` |
...
| `semantic_facts` | the status and reason are one of the pairs a state builder can publish; owners ascend, so one relation publishes one fact set in source order, every window output names a select of the same fact set, and one ambiguous clause dependency blocks the relation for conflicting facts and blocks every window output | `ProjectModuleSemanticFactEnvironment`, `ProjectModuleRelationSemanticFacts` |
