...
| `row_lineage_field` | every retained field keeps at least one complete path, no two paths are identical, field names do not repeat, the field position is the record's own ordinal, since both lineage builders enumerate the complete field collection, and every path starts at that field, stays contiguous hop by hop, and ends at its declared root | `ProjectModuleRowFieldLineage`, `ProjectModuleRelationLineage`, `ProjectModuleRowLineagePath` |
