...
| `row_lineage_path` | a zero-hop path is its own source field, so its enclosing field is a source field, and a path of one hop or more starts at a hop output, so its enclosing field is a relation output | `ProjectModuleRowLineagePath`, `ProjectModuleRowLineageHop` |
