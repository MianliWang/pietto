...
| `semantic_facts` | the owner is a relation declaration of this module; the status and reason are one of the pairs a state builder can publish; owners ascend, so one relation publishes one fact set in source order, every window output names a select of the same fact set, and one ambiguous clause dependency blocks the relation for conflicting facts and blocks every window output | `ProjectModuleSemanticFactEnvironment`, `ProjectModuleRelationSemanticFacts` |
