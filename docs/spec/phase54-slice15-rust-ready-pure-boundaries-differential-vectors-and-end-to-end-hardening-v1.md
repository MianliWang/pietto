...
| `origin` | local origins precede imported ones and ascend by target declaration, so they follow their catalog order; one origin path, including its whole access chain, appears once; the namespace and declaration kind are one of the eight pairs a definition can produce; a local origin is one self path with no hop that targets its own local name in its own module; an imported origin always carries its access chain | `ProjectModuleOriginPath` |
