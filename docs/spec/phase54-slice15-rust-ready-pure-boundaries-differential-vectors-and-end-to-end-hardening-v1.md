...
| `module` | every selected module path appears once, every module identity path in the document is a normalized project-relative `.pietto` path, every graph import evidence names one of the module's own import requests, and the module's row lineages and its semantic fact sets answer for exactly the same declarations, since the attribution and semantic sections are built from one relation collection | `ProjectLogicalModule`, `ProjectModuleIdentity`, `ProjectModuleAttributionFactSet`, `ProjectModuleSemanticFactEnvironment` |
...
| `graph` | the component-member count is positive, any multi-member component proves a cycle, the members are distinct selected modules in selected order, every member publishes this same component, the component contains the module it describes, the dependency targets are distinct and follow selected module order, a cyclic component is exactly a loader-blocked module, a single-member component carries its own self edge exactly when it claims a cycle, and the import evidence names exactly those targets in source order | `ProjectInspectionGraph`, `ProjectModuleStronglyConnectedComponent`, `_derive_inspection` |
| `import` | the namespace and declaration kind form an eligible pair; a supplied resolved target keeps the same namespace and declaration kind; the four resolved-target keys are present together or absent together; and the request has exactly one outcome, so an unresolved request carries at least one issue and a resolved one carries at most the duplicate request issue, which is also the only status its issues may hold, while an unresolved one keeps at least one issue that is not that duplicate; the issues follow their environment's status rank; a resolved request names one declaration of the inspection, by module path, declared name, namespace, and kind, and that declaration is the only occurrence of its identity, since an ambiguous bucket publishes no facade entry to resolve through; the request also names a facade entry of the inspection that exposes the requested name and targets that same declaration, because a resolved binding retains the entry it resolved through; an issue answers for the evidence its request has, so an unresolved target module names a module this inspection did not select, every facade-evidence status names one it did, and a local declaration collision names a declaration this module publishes under that same local name; a request whose target module this inspection selected is one of its module's graph import evidence edges, because the authority publishes an edge for every such request whether or not the exported name resolves; and a resolved request publishes one imported entry of its own module's origin catalog, under the same local name, namespace, and declaration kind, targeting that same module path and declared name, since the catalog is built from the resolved bindings themselves | `ProjectImportedBindingIdentity`, `ProjectResolvedImportedBinding`, `ProjectModuleBindingEnvironment`, `ProjectInspectionImport`, `_derive_binding_authority_graph` |
| `export` | the namespace and declaration kind form an eligible pair; a supplied entry keeps the same namespace and declaration kind, exposes the local name, and, for a local declaration origin, targets that same declared name in the enclosing module; the six facade-entry keys are present together or absent together; and the request has exactly one outcome, so an unentered request carries at least one issue and an entered one carries at most the duplicate request issue, which is also the only status its issues may hold, while an unentered one keeps at least one issue that is not that duplicate; the issues follow the surface's own status rank, which is not the enumeration order; an issue answers for the candidates the module publishes, so an ambiguous local declaration names a declaration the module publishes as ambiguous, and an unresolved binding names no declaration of that exact symbol and no imported binding of it; an entry answers to the candidate that produced it, so a local declaration origin names the module's single declaration occurrence of that name, which the module publishes later, with no imported binding of that name surviving in the module's origin catalog, whatever namespace that binding names, since a local declaration collides with an import by name alone, and a re-export names the resolved target of the import request bound under its own local name, not merely of some request of this module; either way the entry names the only occurrence of one declaration of the inspection, wherever that module sits | `ProjectModuleExportEntry`, `ProjectModuleExportSurface`, `ProjectInspectionExport` |
| `declaration` | the owner kind is the module, its namespace is the reserved empty local namespace, and its name is the enclosing module path; the namespace and declaration kind are one of the eight pairs a definition can produce; the relation status and reason are one atomic pair drawn from the pairs a state builder can publish for that declaration kind, an unknown state carries no row field at all, since an unknown schema is built with an empty field mapping, and a source publishes only the three its own builder reaches — concrete from a resolved shape, unknown from an unresolved one, and blocked from a missing local symbol; the namespace decides the availability domain, a non-relation declaration publishes no relation product, and a retained relation state maps its exact availability; a relation declaration that retained a state publishes exactly one semantic fact set and exactly one row lineage, because the environment and the attribution set each correspond item for item with the complete row-fact collection, and an ambiguous relation identity publishes both even though it retains no state at all; every declaration answers to one local entry of its own module's origin catalog, by declaration position, declared name, namespace, and kind, since the catalog enumerates the module's own definitions before it admits any import; a type alias publishes its alias-base resolution even where its own availability is absent or ambiguous, because the resolver builds one reference for every alias declaration it reaches; an ambiguous identity in the type namespace publishes a type-source issue under its own declared name, since the symbol collector diagnoses a repeated type name rather than resolving it silently, and the status of that issue is not fixed here because a module-level diagnostic can block the symbol before the ambiguity itself is reported; the module readiness decides the availability and the multiplicity together, so a loader-blocked module publishes only blocked declarations and a ready module keeps a repeated identity ambiguous, and a repeated identity publishes its whole bucket, once per index; a positive row-field count requires a schema-bearing relation state, concrete or unknown; in a ready module a unique relation declaration retains its relation state and only a loader-blocked module publishes a blocked declaration without one; the occurrence index is inside its bucket | `ProjectInspectionDeclaration`, `ProjectLayeredDeclarationAsset`, `ProjectLayeredOwnerIdentity` |
| `origin` (target) | an imported or local origin names one exact declaration occurrence of the inspection, by module path, declaration position, declared name, namespace, and kind, wherever that module sits in the stream | `ProjectModuleOriginPath`, `_binding_origin_path` |
| `origin` | a local origin names a declaration of its own module, by position, name, namespace, and kind together; local origins precede imported ones and ascend by target declaration, imported ones ascend by the source position of their first hop, so both follow the order they are built in; one origin path, including its whole access chain, appears once; the namespace and declaration kind are one of the eight pairs a definition can produce; a local origin is one self path with no hop that targets its own local name in its own module; an imported origin always carries its access chain | `ProjectModuleOriginPath` |
| `origin_hop` | the first hop of an imported origin is the import request that origin resolves, under the same local name, namespace, and declaration kind, and that request's resolved target is the origin's own nominal target; each hop is one direct route, so its import target and facade module agree and its exported and exposed names agree; every hop names the origin's own nominal target; the chain re-exports at every interior hop and terminates at a local declaration; the terminating facade is the declaration itself, in its module and under its declared name; and no chain repeats a hop, since a route revisited is a route the contiguity guard cannot have built | `ProjectModuleAccessHop`, `ProjectModuleOriginPath`, `ProjectModuleExportEntry` |
| `row_lineage_path` | a zero-hop path is its own source field, so its enclosing field is a source field, and a path of one hop or more starts at a hop output, so its enclosing field is a relation output; a root in this module is one row field that module publishes, by owner position, field position, and name, and every root, local or not, belongs to a source declaration of the inspection, since every root is a source field | `ProjectModuleRowLineagePath`, `ProjectModuleRowLineageHop` |
| `row_lineage_hop` | the projection kind is decided by the two names: direct keeps the name, renamed changes it; and every retained hop answers to a row-field dependency of its own module for its own reference site, the lineage's owner declaration and this field's position, because the attribution set rebuilds one dependency from every hop it retains | `ProjectModuleRowLineageHop`, `ProjectModuleAttributionFactSet` |
| `row_lineage` (owner kind) | the state pair is one a Slice 10 row fact can carry, which is ten of the nineteen the relation table holds, because the aggregate, group, and window reasons belong to the Slice 12 reduction rather than to a row fact; a state pair no source builder reaches names a table or query owner, since a source reaches only the three its own builder does | `ProjectModuleRelationLineage`, `_source_row_state` |
| `row_lineage` | the owner is a relation declaration of this module; the status and reason are one of the pairs a state builder can publish; a non-concrete lineage carries no field; a concrete lineage carries exactly the row fields its own declaration publishes, since a concrete row fact and a concrete row schema are two projections of one field mapping; only a ready module publishes a lineage at all, because a loader-blocked module reaches no attribution pass; and owners ascend, so one relation publishes one lineage in source order | `ProjectModuleRelationLineage`, `ProjectModuleAttributionFactSet` |
| `source_shape_resolution`, `relation_resolution` | the owner is a declaration of this module of the kind the record admits, a shaped source or a table or query; owners ascend, so one declaration resolves once in source order; the resolved target is one entry of this module's origin catalog, by target module path and declared name, of the kind the reference admits — a shape for a source shape, a source or table or query for a relation — and a relation resolution matches its local name there too; the target is the one unambiguous declaration of that identity in the inspection; and the resolution names the dependency its provenance built, for the two roles that always build one | `ProjectModuleSourceShapeReference`, `ProjectModuleRelationReference` |
| `semantic_facts` | the state is the state its own declaration retained, because both project one Slice 12 fact, unless that declaration retained none at all; a state pair no source builder reaches names a table or query owner; the owner is a relation declaration of this module, a loader-blocked module publishes no fact set at all, and a fact set carrying any derived product names a table or query, since a source keeps its base row state and publishes none, while a fact set that publishes no select at all names a source, because every table and query environment is built from a select list; the status and reason are one of the pairs a state builder can publish; owners ascend, so one relation publishes one fact set in source order, every window output names a select of the same fact set, and one ambiguous clause dependency blocks the relation for conflicting facts and blocks every window output | `ProjectModuleSemanticFactEnvironment`, `ProjectModuleRelationSemanticFacts` |
...
| --- | --- | --- |
| uncle combination | values of this record against one value a sibling collection of an open ancestor already published | a declaration's availability and multiplicity answer to its module's readiness, a cyclic component is exactly a loader-blocked module, a blocked module publishes neither a fact set nor a row lineage, and a cycle issue answers to its module's component |
| ancestor combination | one value of this record against one value of an open ancestor record | a resolved request's only admitted issue is the duplicate request, for both an import and a facade entry, and a concrete relation publishes only concrete window outputs, while a deferred one publishes only deferred outputs, since a deferred final state is either the base state every output then takes or the reduction of outputs that were themselves deferred |
...
| grouped groups agree | the child collections of every scope, ranked by another collected in document order, with a value outside that ledger refused before it is indexed | every member of a graph component publishes that same component, which is what a partition means |
...
| collected implies | one collected child value triggering a state of the scope and of another collection | one ambiguous clause dependency blocks the relation for conflicting facts and blocks every window output |
| group first increasing | the first child of each sibling scope of one kind | imported origins follow their import request order |
...
| sibling buckets complete | the siblings of one kind, grouped by a declared key tuple, read in the order the scope published them | a repeated nominal identity publishes its whole occurrence bucket, once per index, and each clause role publishes a dense source ledger |
| distinct siblings | the sibling key tuples already seen in this scope | an alias chain, a component, a cycle, a request's issue statuses, a module's dependency facts, and the hops of one origin chain each forbid a duplicate member |
...
rule already used. A rule may also be restricted by a key of the record it
governs: by that key's value, by a conjunction of values, or, where the key is
optional and its value is a path or a name no rule can enumerate, by presence
alone. Presence is the same substitution a combination already makes for an
ancestor key. One more restriction reads the open scope: a rule may govern only
those records whose named key equals the enclosing scope's, which is how a
reference that may point into another module is bound in the case this document
can witness — the local one — and skipped in the case it cannot.
...
A competing export candidate is counted from the origin catalog, not from the
import requests, and the self-import fixture is why. A module that imports its
own name collides, so the request resolves to nothing and publishes no imported
binding, while the local declaration still earns the entry; an exclusion over
import requests would reject that projection. A module that imports a name it
also declares collides the same way. What the authority forbids is a surviving
*candidate*, which is exactly an imported binding in the catalog.

A relation's semantic outputs are not matched against its row schema. The
grouped fixture publishes a fact set with three selects for a relation whose
declaration publishes no row fields at all, so a rule tying a select's output
name, or a window output's, to the declared row schema would reject the
authority's own bytes. Two more issue statuses stay free for the same reason:
an import binding collision names no local declaration — two imports of one
name collide with each other, which the two-candidate fixture projects — and a
candidate-set ambiguity counts candidates rather than naming one.

An origin is not required to name an unambiguous declaration, and the ambiguous
export fixture refutes it: a module that declares one name twice publishes one
local origin per occurrence, so the catalog points into the ambiguous bucket by
design. The obligation belongs to the records that resolve *through* the
catalog — a resolution, a dependency target, an alias, an import, a facade entry
— and each of those carries it.

A row lineage's state is not its owner declaration's retained state, and the
rich fixture refutes that equality: the query publishes a deferred Slice 11
lineage while its declaration keeps the concrete Slice 12 state its relation
earned. A semantic fact set is the opposite case and now carries the equality,
because the declaration retains that very fact's state — with one exception the
ambiguous-relation fixture publishes: a declaration that retained no state at
all answers for nothing, so the match admits a withheld pair.

A row lineage's field count is not, in general, its owner declaration's row
field count, and the rich fixture refutes the unguarded equality directly: the
query publishes a deferred lineage with no fields while its declaration keeps
the three-field row schema its relation state earned. The counts describe two
products of two slices, and only one of them is empty. Where the lineage itself
is concrete the two are one field mapping projected twice, and that is exactly
how the rule is declared: guarded by the lineage's own state, never by the
declaration's. A readiness cycle is likewise not unique within its
...
deferred Slice 11 lineage while its Slice 12 relation state is concrete, which
the rich bootstrap fixture does; and an ambiguous nominal identity publishes no
relation state at all while its fact set still carries one. The declaration
retains a relation state only for a ready, unique, relation declaration, so the
equality holds on a subset of cases and is therefore not an invariant. What is
one: a loader-blocked module publishes no fact set at all, because the blocked
branch of the availability atomicity guard forbids every semantic product.

The ambiguity diagnosis is bound by name and family, never by status. Two
fixture projections publish an ambiguous shape bucket whose only type-source
issue is a module diagnostic blocked under that same name rather than the
ambiguity status itself, so a rule naming `ambiguous_local_type_name` would
reject a document the authority produced. The relation namespace stays unbound
in this direction as well: nineteen of its twenty-three ambiguous buckets
publish no type-source issue of their own name at all, because a relation
ambiguity is diagnosed by the relation surface instead.

Completeness runs one way only, and three near neighbours of the rules above
are refused for that reason. A shape declaration does not owe a shape-field
type resolution, because a shape may declare no field at all and the empty
fixture projects exactly that; no availability guard recovers the rule either,
since the empty shape is as concrete as any other. A type reference whose
direct kind is a builtin or unknown does not owe a dependency, because the
provenance builds a dependency fact only where the reference reaches a declared
symbol — the guard on the mirror rule carries the reference's own direct kind
for that reason, and dropping it rejects fifteen of the nineteen fixture
projections. And the import-issue correspondence has no converse: an import
issue naming an unresolved target module is raised per request, while the graph
raises at most one issue per module, so a module with two unresolved requests
publishes two import issues and one graph issue. The rule declared is the
direction the evidence supports, and only that direction.

Two orderings stay undeclared because the evidence that decides them is not in
the projection at all. An alias chain's order follows each alias's own base,
resolved through a retained authority mapping, and neither the direct symbol's
identity nor any alias's base is a serialized value; a shape field type's member
position corresponds to a field of the owning shape, and a shape declaration
publishes no field list in the stream. A rule for either would have to invent
its evidence, which is exactly what this table refuses.

An origin chain's interior facade is not matched against the module that
publishes it. Every hop states its own route — the import target and the facade
module agree, the exported and exposed names agree, an interior hop re-exports
and the last one terminates at the declaration itself — but that the facade
module really imports the next module and really re-exposes the symbol is a
claim about *another* module's import and export sections. Those records carry
their module's ordinal, not its path, and no record joins the two, so a
cross-module ledger cannot be keyed by the path a hop carries; and a module may
name a facade the stream has not reached yet. The corpus answers this the only
way the layer can: the re-export vector publishes the whole chain, so every
interior facade in it is a module of the same document with its own import,
export, and origin.

A type resolution's canonical target in another module is not matched against
the origin catalog, and a fixture refutes that rule directly. A module that imports an alias whose
own base is imported from a third module publishes a canonical target in that
third module: `main` imports `type Aliased` from `m1`, `m1` aliases an enum it
imported from `m2`, and `main`'s resolution names `m2`. `main`'s catalog holds
only its own declarations and `m1`'s `Aliased`, so the match would reject a
document the authority produced. The canonical target is reached through the
chain, not through this module's bindings, and the chain's interior is not in
the stream.

A facade ledger sits beside it, and it is weaker in one exact way. An export
record carries the identity it targets but not the path of the module that
publishes it, only that module's ordinal, so a resolved request can be matched
to a facade that exposes its name and targets its declaration, and not to the
facade of the one module it imported from. Where a chain re-exports, those are
different modules. The stronger claim would need the same ordinal-to-path join
the interior facade case already refuses.

The cross-module ledger the inspection holds is keyed by a declaration's own
module path, which the record carries as its owner name. That is what lets a
resolved import, a facade entry, an origin, a canonical type target, and a
lineage root owner all name one exact declaration of the inspection rather than
merely a selected module. It reaches only declarations: a row field carries its
module's ordinal and not its path, so nothing keyed on a field can be joined the
...
The relation status and reason table is the one rule derived from construction
sites rather than from a single carrier's validation: every state builder in
the resolution, aggregate, and window paths pairs each reason with exactly one
status family, and the aggregate finalization classifier validates its own
partition explicitly. The reserved table-upstream reason has no construction
site anywhere, so no status admits it.
...
singleton; duplicate singleton; module count mismatch; an impossible state
combination; and one omission per collection a record's own state obliges it to
publish — a declaration without its local origin, a resolved import without its
