...
| `graph` | the component-member count is positive, any multi-member component proves a cycle, the members are distinct, the component contains the module it describes, the dependency targets are distinct, a cyclic component is exactly a loader-blocked module, a single-member component carries its own self edge exactly when it claims a cycle, and the import evidence names exactly those targets in source order | `ProjectInspectionGraph`, `ProjectModuleStronglyConnectedComponent`, `_derive_inspection` |
