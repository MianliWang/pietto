...
six-kind export matching, a caller-supplied explicit named re-export seam, and
...
re-exports. Slice 10 adds a second private dependency-first resolution set for
the exact source/table/query relation namespace, `TableDef.from_clause` and
...
identity, occurrence order, multiplicity, or availability state. Slice 13 layers
...
The Phase 27 target is grouped result-scope `ORDER BY` over bare selected
output names. Accepted names may refer to selected group-key projection
outputs, selected direct aggregate projection outputs, and selected Phase 26
aggregate-expression projection outputs such as `sum(amount + tax)`,
`avg(score * weight)`, and `count_distinct(lower(trim(status)))`.

SQL lowering renders the underlying selected expression rather than the SELECT
alias. The MVP keeps `PIE-S2321` as the unsupported grouped `order by:`
diagnostic family and does not add a new diagnostic code.

Phase 27 adds no arbitrary grouped `ORDER BY` expressions, direct aggregate
calls inside source `order by:`, ordinal ordering, no-GROUP projection-alias
ordering, broad `ORDER BY` / `LIMIT` redesign, grammar, generated ANTLR, AST,
...
`ORDER BY` / `LIMIT` redesign, no-GROUP projection-alias ordering, direct
aggregate calls inside source `order by:`, ordinal ordering, and arbitrary
grouped order expressions remain deferred.
...
The completed Phase 28 behavior admits only Int and Float numeric literal
leaves inside selected `sum(...)` and `avg(...)` numeric expression arguments.
Accepted expressions must still include at least one direct input field leaf.
...
typing. `sum(Int expression)` keeps the existing `sum` Int result behavior,
`sum(Float expression)` keeps the existing Float result behavior, and
`avg(Int or Float expression)` keeps the existing avg result behavior. Mixed
Int/Float expression behavior follows existing scalar numeric typing and does
not introduce a new promotion system.

Phase 28 adds no Decimal literal, Decimal multiplication, Decimal division,
mixed Decimal/Int or Decimal/Float promotion, casts, precision/scale modeling,
schema introspection, arbitrary scalar calls inside `sum` / `avg`, division,
modulo, `count(expression)`, `min(expression)`, `max(expression)`,
`count_distinct(...)` widening, grammar, generated ANTLR, AST, parser, IR
model, SQL fixture/golden, JSON schema, CLI option, dependency, public API,
...
29 defines v0.2 as a stable single-file typed SQL authoring compiler. Phase
30 Core Type System Stabilization I is complete, Phase 31 v0.2 Hardening And
Stable Completion is complete, and Pietto v0.2 single-file stable is complete
as an internal compiler boundary.

The accepted merged Phase 31 mainline is Phase 31 v0.2 Hardening And Stable
Completion. Phase 31 Slice 8 is complete as v0.2 Stable Completion Audit And
Status Lock. Phase 32 is post-v0.2 Semantic Explain And Metadata Output MVP.
...
eight-slice master plan. Slice 2 is complete as canonical scalar type registry
...
three-valued logic `UNKNOWN` remain distinct. Slice 4 is complete as Bool and
predicate semantics contract, static audit, and status work only. Known Bool
predicate acceptance remains a compile-time type-level fact and does not
imply non-null proof, runtime truth, or SQL three-valued logic collapse.
Slice 5 is complete as Date / Timestamp formalization contract, static audit,
and status work only. `Timestamp` is the current canonical v0.2 spelling for
date+time values; Slice 5 records current generic comparison behavior only and
...
Phase 30 is complete, and Phase 31 v0.2 Hardening And Stable Completion is
complete as the merged v0.2 status-lock phase. Phase 31 Slice 8 locks Pietto
v0.2 single-file stable complete for the internal compiler boundary only.
...
`182ed41e7dc7dd7e616cfb1be5cfbb4a7fcdae58`, final Phase 30 commit `Complete
Phase 30 core type system stabilization audit`, and CI run `27891119809
success`, and adds the eight-slice Phase 31 master plan. Phase 29 deferred
register remains active, Phase 29 aggregate freeze remains active, and Phase
30 type-system contracts are carried forward.

The eight-slice Phase 31 plan is Candidate Decision And Phase 30
Carry-forward Audit, Aggregate Result Matrix Hardening, Numeric Promotion And
...
Audit And Status Lock. Slice 8 is complete.
...
introspection, or type-system behavior changes. It adds no aggregate
expansion, JSON v1 schema expansion, JSON v2, public MySQL API expansion,
...
release tag, publishing, or v0.2 completion declaration in Slice 1. Phase 31
is complete. Phase 31 Slice 2 Aggregate Result Matrix Hardening is
...
is the internal single-file stable compiler boundary; it is complete as of
Phase 31 Slice 8 after repository-local validation and status lock. `0.1.0`
is the current package and installed CLI version. Slice 7 locks docs, examples, package,
...
explain` CLI behavior was implemented in Slice 1. Phase 36 Post-v0.2 Core Type
System Expansion MVP is complete as of Slice 12 Completion Audit And Status
Lock. Slice 12 is the final completion audit/status lock. Slice 5 is the only
Phase 36 behavior change: `count(Enum field)` now fails closed in semantic
aggregate validation with `PIE-S2314`. The Phase 36 type-candidate resolutions
are: Decimal precision-scale carrier deferred with exact prerequisites; UUID
`limited_frozen` with no behavior expansion; Enum metadata/readiness except the
Slice 5 fail-closed fix with `PIE-S2314`; DateTime, Time, and Interval
unsupported/deferred; Any, Bytes, and Json posture documented without behavior
expansion; type alias current behavior preserved while domain refinement
remains deferred; expanded scalar/operator matrix documented without behavior
change; and public surface stability locked without behavior change.
Currency/Money and native DB metadata remain deferred. No public schema/output
expansion occurred. Package version remains `0.1.0`. No
tag/release/publish/upload/signing/attestation occurred.
`docs/spec/pietto-v0.9.md` remains a spec/status document path, not the package
version or a release tag. Active roadmap: Phase 32:
...
implemented in Slice 1. Phase 32 Slice 2 Semantic Metadata Artifact v1 Contract
is complete. Slice 2 is docs/spec/static-audit/contract-only. No `pietto
explain` CLI behavior was implemented in Slice 2, and no source, CLI, JSON v1,
semantic, IR, SQL, diagnostic, fixture, golden, example, package, dependency,
workflow, version, release, tooling, tag, publish, upload, signing, or
attestation behavior changed. Phase 32 Slice 3 Private Metadata Model And
Builder MVP is complete. Slice 3 adds only private metadata model/builder
source, tests, status, and hash-lock updates. Phase 32 as a whole is not
...
signing, or attestation behavior changed. Phase 32 Slice 6 JSON Serializer And
...
v2/project/multi-file implementation behavior was added, no
...
green. Phase 33 Slice 4 Private Project Discovery Model MVP is complete. Phase
33 Slice 5 Project Check CLI MVP is complete. Phase 33 Slice 6 JSON v2
Serializer MVP is complete. Phase 33 Slice 7 Project Explain/Metadata
Aggregation Contract is complete as docs/spec/static-audit/status-only work.
Phase 33 Slice 8 CLI, Package Smoke, Docs, And Compatibility Hardening is
complete. Phase 33 Slice 9 Completion Audit And Status Lock is complete. Phase
33 JSON v2 And Project / Multi-file MVP is complete. Phase 33 delivered a
conservative project-mode foundation: private `_project` model/discovery source,
text-mode `pietto check --project ROOT` root/config validation, project JSON v2
for `pietto check --project ROOT --format json`, project explain / metadata
aggregation boundary contract, and package smoke / compatibility hardening.
...
`PIE-S2314`. The Phase 36 type-candidate resolutions are: Decimal
...
change; and public surface stability locked without behavior change.
Currency/Money and native DB metadata remain deferred. No public schema/output
expansion occurred. CLI JSON v1, Project JSON v2, Semantic Metadata Artifact v1
schema/output, diagnostic envelope, SQL golden bytes, fixtures/goldens,
workflows, scripts, lockfiles, and package metadata are unchanged. Slice 12
authorizes no source/compiler behavior changes, workflow changes, release
operations, public schema changes, fixtures/goldens, scripts, or generated
artifacts. Package version remains `0.1.0`. No
tag/release/publish/upload/signing/attestation occurred.
`docs/spec/pietto-v0.9.md` remains a spec/status document path, not the package
version or a release tag.
...
## 19. Phase 55 Slice 3 Package-manifest Status
...
Slice 2 schema-v3 activation carrier remain exact. It adds no package identity,
digest, asset catalog, locator, loading, dependency, public JSON/CLI/API/IR/SQL,
or release behavior.
