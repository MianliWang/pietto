...
The implementation uses a private semantic checker and the existing ordered
diagnostic result. It adds only `PIE-S2601`, `PIE-S2602`, and `PIE-S2603`.
Relationship metadata remains outside existing semantic namespaces and the
...
Relationship metadata produces no IR definition and no SQL artifact. CLI text
...
Later Phase 15 work requires separate authorization. Slice 1 does not
